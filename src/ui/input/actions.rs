...
pub fn map_to_game_action(event: &Event, bindings: &KeyBindings) -> Option<GameAction> {
    match event {
        ...
        // 组合键实现斜向移动（参考像素地牢PC版）
        Event::Key(KeyEvent { code: KeyCode::Up | KeyCode::Left, .. }) => Some(GameAction::MoveUpLeft),
        Event::Key(KeyEvent { code: KeyCode::Up | KeyCode::Right, .. }) => Some(GameAction::MoveUpRight),
        Event::Key(KeyEvent { code: KeyCode::Down | KeyCode::Left, .. }) => Some(GameAction::MoveDownLeft),
        Event::Key(KeyEvent { code: KeyCode::Down | KeyCode::Right, .. }) => Some(GameAction::MoveDownRight),
        ...
        Event::Key(KeyEvent { code: KeyCode::Char('0'), .. }) => Some(GameAction::QuickSlot(9)),
...
pub fn map_to_ui_action(event: &Event, bindings: &KeyBindings) -> Option<UIAction> {
    match event {
        ...
        Event::Key(KeyEvent { code: KeyCode::Tab, .. }) => Some(UIAction::TabNext),
        
        // 翻页
        Event::Key(KeyEvent { code: KeyCode::PageUp, .. }) => Some(UIAction::PageUp),
        Event::Key(KeyEvent { code: KeyCode::PageDown, .. }) => Some(UIAction::PageDown),
        
        // 系统功能
        Event::Key(KeyEvent { code: KeyCode::F5, .. }) => Some(UIAction::QuickSave),
        Event::Key(KeyEvent { code: KeyCode::F9, .. }) => Some(UIAction::QuickLoad),
        Event::Key(KeyEvent { code: KeyCode::F11, .. }) => Some(UIAction::ToggleFullscreen),
...
mod tests {
    ...
    fn test_default_keybindings() {
        ...
        // 测试数字键快捷栏
        assert_eq!(
            map_to_game_action(&Event::Key(KeyEvent::new(KeyCode::Char('1'), KeyModifiers::NONE)), &bindings),
    ...
    fn test_ui_actions() {
        ...
        // 测试标签页切换
        assert_eq!(
            map_to_ui_action(&Event::Key(KeyEvent::new(KeyCode::Tab, KeyModifiers::SHIFT)), &bindings),
            Some(UIAction::TabPrev)
        );
    }
}
