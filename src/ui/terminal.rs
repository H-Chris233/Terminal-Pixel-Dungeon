    ...
    pub fn handle_input(&self) -> Result<Option<GameAction>> {
        if event::poll(std::time::Duration::from_millis(100))? {
            if let Event::Key(key) = event::read()? {
                match key.code {
                    KeyCode::Char('h') | KeyCode::Left => return Ok(Some(GameAction::Move(Direction::West))),
                    KeyCode::Char('j') | KeyCode::Down => return Ok(Some(GameAction::Move(Direction::South))),
                    KeyCode::Char('k') | KeyCode::Up => return Ok(Some(GameAction::Move(Direction::North))),
                    KeyCode::Char('l') | KeyCode::Right => return Ok(Some(GameAction::Move(Direction::East))),
