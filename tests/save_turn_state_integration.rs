...
use save::{SAVE_VERSION, SaveSystem};
...
fn test_save_and_restore_turn_state() {
    ...
    ecs_world
        .generate_and_set_dungeon(5, 12345)
    ...
    // Save the game
    let save_data = ecs_world
        .to_save_data(&turn_system)
    ...
    let (restored_turn_state, restored_action_taken) = ecs_world
        .from_save_data(loaded_data)
    ...
    assert_eq!(
        turn_system.state, original_turn_state,
        "Turn state not restored correctly"
    );
    assert_eq!(
        turn_system.player_action_taken(),
        true,
        "Player action taken flag not restored"
    );

    // Verify clock state was restored
    assert_eq!(
        ecs_world.resources.clock.turn_count, original_turn_count,
        "Turn count not restored"
    );
    assert_eq!(
        ecs_world.resources.clock.elapsed_time.as_secs(),
        original_elapsed,
        "Elapsed time not restored"
    );
...
fn test_backward_compatibility_v1_saves() {
    ...
    let mut ecs_world = ECSWorld::new();
    ecs_world
        .generate_and_set_dungeon(3, 54321)
        .expect("Failed to generate dungeon");
    ...
    assert_eq!(
        loaded_data.turn_state.current_phase,
        save::TurnPhase::PlayerTurn
    );
...
fn test_save_mid_combat_with_enemies() {
    ...
    ecs_world
        .generate_and_set_dungeon(3, 99999)
        .expect("Failed to generate dungeon");
    ...
    let save_data = ecs_world
        .to_save_data(&turn_system)
        .expect("Failed to save mid-combat");
    save_system
        .save_game(2, &save_data)
        .expect("Failed to save to slot 2");
    ...
    // Load and verify
    let loaded = save_system
        .load_game(2)
        .expect("Failed to load combat save");
